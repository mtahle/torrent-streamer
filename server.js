...
app.post("/start", async (req, res) => {
  ...
  try {
    ...
    if (active.torrent) {
      ...
      active = { torrent: null, file: null, fileIndex: null, movieId: null, sessionId: null, metadata: { title: null, quality: null, year: null } };
    }
    ...
    const existingTorrent = client.torrents.find(t => 
      t.magnetURI === magnet || t.infoHash === magnet.match(/btih:([a-f0-9]{40})/i)?.[1]
    );
    
    if (existingTorrent) {
      console.log('⚠️  Torrent already exists, reusing:', existingTorrent.infoHash);
      
      // Use existing torrent
      const file = existingTorrent.files
      ...
      const session = await DatabaseUtils.createActiveSession(movie.id, sessionData);
      ...
      active = {
        ...
        metadata: { title: movie.title, quality: movie.quality, year: movie.year }
      };
    ...
    const timeoutId = setTimeout(() => {
      if (!res.headersSent && !responseHandled) {
        ...
        console.error('Torrent add timeout - callback not fired within 60 seconds');
        res.status(500).json({ error: 'Torrent initialization timeout. The torrent may take longer to start. Please try again.' });
      }
    }, 60000);

    const torrentInstance = client.add(magnet, { path: "./data" }, async (torrent) => {
      try {
        ...
        await movie.reload();
        console.log('🔍 Movie infoHash:', movie.infoHash, 'Torrent infoHash:', torrent.infoHash);
        ...
        await DatabaseUtils.updateMovieStatus(movie.id, 'downloading', updateData);
        ...
        clearTimeout(timeoutId);
        if (!res.headersSent) {
          res.status(500).json({ error: 'Failed to setup torrent', details: error.message });
        }
      }
    });
    ...
    client.on('error', async (error) => {
      ...
      if (!res.headersSent) {
        res.status(500).json({ error: 'WebTorrent client error', details: error.message });
      }
    });
    ...
    console.error('Error stack:', error.stack);
    if (!res.headersSent) {
      res.status(500).json({ error: 'Failed to start torrent', details: error.message });
    }
  }
});
  ...
  try {
    ...
    const movies = await models.Movie.findAll({
      ...
      include: [
        ...
        { model: models.WatchHistory, as: 'watchHistory', limit: 1, order: [['watchedAt', 'DESC']] }
      ]
    });
...
app.post("/api/dlna/control", async (req, res) => {
  ...
  if (!action) {
    return res.status(400).json({ error: 'action required (play, pause, stop, seek)' });
  }
    ...
    switch (action) {
      ...
      case 'seek':
        if (typeof position !== 'number') {
          return res.status(400).json({ error: 'position (in seconds) required for seek' });
        }
        await dlnaManager.seek(position);
        break;
      default:
        return res.status(400).json({ error: 'Invalid action. Use: play, pause, stop, or seek' });
    }
...
app.post("/api/airplay/control", async (req, res) => {
  ...
  if (!action) {
    return res.status(400).json({ error: 'action required (play, pause, stop, seek, volume)' });
  }
...
app.post("/api/rtp/start", async (req, res) => {
  ...
  if (!ffmpegAvailable) {
    return res.status(503).json({ 
      ...
      message: 'Please install FFmpeg to use RTP streaming: sudo apt-get install ffmpeg' 
    });
  }
  ...
  try {
    ...
    const streamInfo = await rtpStreamer.startRTPStream(active.file, active.metadata, options);
...
app.post("/api/udp/start", async (req, res) => {
  ...
  if (!ffmpegAvailable) {
    return res.status(503).json({ 
      error: 'FFmpeg not installed', 
      message: 'Please install FFmpeg to use UDP streaming: sudo apt-get install ffmpeg' 
    });
  }
  ...
  try {
    ...
    const streamInfo = await rtpStreamer.startUDPStream(active.file, active.metadata, options);
...
app.get("/api/udp/status", (req, res) => {
  try {
    const streams = rtpStreamer.getAllStreams().filter(s => s.id.startsWith('udp-'));
    const sapAnnouncements = sapAnnouncer.getActiveAnnouncements();
